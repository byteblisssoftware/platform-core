...
insert into public.role_permissions (role, permission) values
  ...
  ('super_admin','conversation.manage'),
...
create or replace function public.chat_profile_defaults()
returns trigger language plpgsql security definer set search_path = public as $$
begin
  update public.profiles p
  set handle = coalesce(p.handle, new.raw_user_meta_data->>'handle', 'user-' || substr(new.id::text, 1, 8)),
      display_name = coalesce(p.display_name, new.raw_user_meta_data->>'display_name', p.full_name, p.username, 'Member'),
      job_title = coalesce(p.job_title, new.raw_user_meta_data->>'job_title')
  where p.id = new.id;
  return new;
end;
$$;
revoke all on function public.chat_profile_defaults() from public, anon, authenticated;
create trigger zz_chat_profile_defaults after insert on auth.users for each row execute function public.chat_profile_defaults();
